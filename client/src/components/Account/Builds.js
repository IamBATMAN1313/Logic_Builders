...
export default function Builds() {
  ...
  const openComponentSelector = (categoryName) => {
    // Category name to ID mapping based on API response
    const categoryMapping = {
      'CPU': 7,
      'Cpu': 7,
      'Memory': 14,
      'Motherboard': 16,
      'Storage': 8, // Using External Hard Drive for storage
    ...
    // Handle categories that have multiple subcategories
    if (categoryName === 'Storage') {
      // For now, just navigate to External Hard Drive since Internal Hard Drive doesn't exist
      // In the future, we could show options for different storage types
      navigateToCategoryById(categoryMapping['External Hard Drive']);
    } else if (categoryName === 'CPU') {
      // Map CPU to the actual category ID
      navigateToCategoryById(categoryMapping['CPU']);
    } else {
      // For single categories, navigate directly using ID
      const categoryId = categoryMapping[categoryName];
      if (categoryId) {
        navigateToCategoryById(categoryId);
      } else {
  ...
  const navigateToCategoryById = (categoryId) => {
    // Navigate to category products page with build context using correct route format
    navigate(`/category/${categoryId}?returnTo=build&buildId=${showBuildDetails}`);
  };
          ...
          {builds.map((build) => (
            <div key={build.id} className="build-card">
              ...
              <div className="build-image">
                <img 
                  src={build.image_url || "/pcbuild.jpg"}
                  alt={build.name}
                  onError={(e) => {
                    e.target.src = "/pcbuild.jpg";
                  }}
                />
              </div>
