...
router.get('/products/price-range', authenticateAdmin, requireClearance('PRODUCT_MANAGER'), async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
    ...
    res.json({
      ...
      max_price: parseFloat(result.rows[0].max_price) || 10000
    });
  } catch (error) {
...
// Get price range for inventory
router.get('/inventory/price-range', authenticateAdmin, requireClearance('INVENTORY_MANAGER'), async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
    ...
    console.error('Error fetching inventory price range:', error);
    res.status(500).json({ error: 'Failed to fetch price range' });
  }
});
...
// Get price range for orders
router.get('/orders/price-range', authenticateAdmin, requireClearance('ORDER_MANAGER'), async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
        COALESCE(MIN(total_price), 0) as min_total,
        COALESCE(MAX(total_price), 5000) as max_total
      FROM "order"
    `);

    res.json({
      min_total: parseFloat(result.rows[0].min_total) || 0,
      max_total: parseFloat(result.rows[0].max_total) || 5000
    });
  } catch (error) {
    console.error('Error fetching order price range:', error);
    res.status(500).json({ error: 'Failed to fetch price range' });
  }
});
...
router.get('/orders', authenticateAdmin, requireClearance('ORDER_MANAGER'), async (req, res) => {
  try {
    const { page = 1, limit = 20, status, payment_status, date_from, date_to, min_total, max_total } = req.query;
    const offset = (page - 1) * limit;
    ...
    if (min_total && !isNaN(parseFloat(min_total))) {
      paramCount++;
      whereClause += ` AND o.total_price >= $${paramCount}`;
      values.push(parseFloat(min_total));
    }

    if (max_total && !isNaN(parseFloat(max_total))) {
      paramCount++;
      whereClause += ` AND o.total_price <= $${paramCount}`;
      values.push(parseFloat(max_total));
    }
