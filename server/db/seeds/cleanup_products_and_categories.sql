-- Script to delete products without images and cleanup related data
-- This script will:
-- 1. Delete all related records for products without images
-- 2. Delete products without images
-- 3. Delete categories that no longer have any products
...
-- 8. Delete from order_item
DELETE FROM order_item 
WHERE product_id IN (SELECT id FROM products_to_delete);

-- 9. Delete from ratings
DELETE FROM ratings 
WHERE product_id IN (SELECT id FROM products_to_delete);
